    ...
    protected function getTableActions(): array
    {
        return [
                    ...
                    Section::make('Detail Pengajuan')
                        ->schema([
                            ...
                            Textarea::make('catatan_revisi')
                                ->label('Catatan Approval Sebelumnya')
                                ->disabled(),
                        ])
                        ->collapsible()
                        ->collapsed(),
                    ...
                    Section::make('Hasil Keputusan atas Revisi')
                        ->schema([
                            Grid::make(2)->schema([
                                ...
                                TextInput::make('revisi_direktur_utama_decision')
                                    ->label('Keputusan Direktur Utama atas Revisi')
                                    ->disabled()
                                    ->default(fn($get) => $get('revisi_direktur_utama_decision') ?? 'Tidak Diketahui'),
                                Textarea::make('revisi_direktur_utama_catatan')
                                    ->label('Catatan Direktur Utama atas Revisi')
                                    ->disabled()
                                    ->default(fn($get) => $get('revisi_direktur_utama_catatan') ?? 'Tidak ada catatan'),
                                TextInput::make('revisi_direktur_utama_approver_name')
                                    ->label('Disetujui oleh Direktur Utama')
                                    ->disabled()
                                    ->default(fn($get) => $get('revisi_direktur_utama_approver_name') ?? 'Tidak Diketahui'),
                            ]),
                        ])
                        ->visible(fn($get) => !empty($get('revisi_kadiv_ga_decision')) ||
                            !empty($get('revisi_direktur_operasional_decision')) ||
                            !empty($get('revisi_direktur_utama_decision')))
                        ->collapsible()
                        ->collapsed(),
                        ...
                        ->schema([
                            ...
                            Grid::make(2)->schema([
                                ...
                                ->visible(fn($get) => !empty($get('direktur_operasional_decision_type')) ||
                                    !empty($get('direktur_operasional_catatan'))),
                            Grid::make(2)->schema([
                                ...
                                ->visible(fn($get) => !empty($get('direktur_utama_decision_type')) ||
                                    !empty($get('direktur_utama_catatan'))),
                        ])
                        ->visible(fn(Pengajuan $record) => in_array($record->status, [
                            Pengajuan::STATUS_MENUNGGU_PENCARIAN_DANA,
                            Pengajuan::STATUS_MENUNGGU_APPROVAL_DIREKTUR_UTAMA,
            ...
            Action::make('approve_revisi')
                ...
                ->requiresConfirmation()
                ->form([
                    Hidden::make('revisi_direktur_utama_decision_type')
                        ->default('Disetujui'),
                ...
                ->action(function (array $data, Pengajuan $record) {
                    ...
                    $revisi->update([
                        'revisi_direktur_utama_decision_type' => $data['revisi_direktur_utama_decision_type'] ?? 'Disetujui',
                        'revisi_direktur_utama_catatan' => $data['revisi_direktur_utama_catatan'] ?? null,
                        'revisi_direktur_utama_approved_by' => Auth::id(),
                    ]);

                    $catatanTambahan = "\n\n[Revisi disetujui oleh Direktur Utama: " . Auth::user()->nama_user . " pada " . now()->format('d-m-Y H:i') . "]";
                    $catatanRevisi = ($record->catatan_revisi ?? '') . $catatanTambahan;
                    ...
                    Log::info('Revisi pengajuan disetujui oleh Direktur Utama', [
                        'id_pengajuan' => $record->id_pengajuan,
                        'revisi_id' => $revisi->id,
                        'status' => Pengajuan::STATUS_MENUNGGU_PENCARIAN_DANA,
                        'approved_by' => Auth::id(),
                ...
                ->visible(fn(Pengajuan $record) => $record->status === Pengajuan::STATUS_MENUNGGU_APPROVAL_DIREKTUR_UTAMA_REVISI),
            ...
            Action::make('reject_revisi')
                ...
                ->action(function (array $data, Pengajuan $record) {
                    ...
                    $revisi->update([
                        'revisi_direktur_utama_decision_type' => $data['revisi_direktur_utama_decision_type'] ?? 'Ditolak',
                        'revisi_direktur_utama_catatan' => $data['revisi_direktur_utama_catatan'],
                        'revisi_direktur_utama_approved_by' => Auth::id(),
                    ]);

                    $catatanTambahan = "\n\n[Revisi ditolak oleh Direktur Utama: " . Auth::user()->nama_user . " pada " . now()->format('d-m-Y H:i') . "]";
                    $catatanRevisi = ($record->catatan_revisi ?? '') . $catatanTambahan;
                    ...
                    Log::info('Revisi pengajuan ditolak oleh Direktur Utama', [
                        'id_pengajuan' => $record->id_pengajuan,
                        'revisi_id' => $revisi->id,
                        'status' => Pengajuan::STATUS_DITOLAK_DIREKTUR_UTAMA,
                        'approved_by' => Auth::id(),

...
class PengajuanPerDivisiChart extends ChartWidget
{
    ...
    protected function getData(): array
    {
        ...
        $data = Pengajuan::query()
            ->join('bprs_sso.users', 'pengajuans.pemohon_id', '=', 'bprs_sso.users.id')
            ->join('bprs_sso.divisis', 'bprs_sso.users.divisi_id', '=', 'bprs_sso.divisis.id')
            ->where('pengajuans.status', Pengajuan::STATUS_SELESAI)
            ->where('pengajuans.updated_at', '>=', $start)
            ->groupBy('bprs_sso.divisis.nama_divisi')
            ->select(DB::raw('bprs_sso.divisis.nama_divisi as divisi'), DB::raw('sum(pengajuans.total_nilai) as total'))
            ->pluck('total', 'divisi');

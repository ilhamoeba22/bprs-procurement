        ...
        return [
            ...
            TextColumn::make('tindakan_saya')
                ...
                ->state(fn(Pengajuan $record): string => match ($record->status) {
                    ...
                    default => $record->ga_id === Auth::id() ? 'Sudah Disurvei' : 'Menunggu Aksi',
                })
                ...
                ->schema([
                    ...
                    Repeater::make('item_details')
                        ...
                        ->schema([
                            ...
                            Hidden::make('kuantitas')->default(1),
                            Hidden::make('item_id'),
                            Grid::make(2)->schema([
                    ...
                    Placeholder::make('total_biaya_vendor')
                        ...
                        ->content(function (Forms\Get $get): HtmlString {
                            ...
                            foreach ($items as $item) {
                                $qty = (float) ($item['kuantitas'] ?? 1);
                                $hargaSatuan = (float) ($item['harga'] ?? 0);
                                $pajak = (float) ($item['nominal_pajak'] ?? 0);
                                $pajakDitanggungPerusahaan = ($item['kondisi_pajak'] === 'Pajak ditanggung Perusahaan (Exclude)' ? $pajak : 0);

                                $total += ($hargaSatuan * $qty) + $pajakDitanggungPerusahaan;
                            }
                        ...
                        TextInput::make('nominal_dp')
                            ...
                            ->maxValue(function (Forms\Get $get): float {
                                ...
                                foreach ($items as $item) {
                                    ...
                                    $pajak = (float) ($item['nominal_pajak'] ?? 0);
                                    $pajakExclude = ($item['kondisi_pajak'] === 'Pajak ditanggung Perusahaan (Exclude)' ? $pajak : 0);

                                    $total += $totalHargaBarang + $pajakExclude;
                                }
    ...
    protected function getSurveyActionLogic(): callable
    {
        return function (array $data, Pengajuan $record, string $successMessage): void {
            ...
            foreach ($data['survei_per_vendor'] as $vendorSurvey) {
                ...
                foreach ($items as $item) {
                    $itemDetail = collect($vendorSurvey['item_details'])->where('item_id', $item->id)->first();
                    if (!$itemDetail) {
            ...
            VendorPembayaran::where('pengajuan_id', $record->id)->delete();
        ...
        return [
            ViewAction::make()->label('Detail')
                ...
                ->mountUsing(function (Form $form, Pengajuan $record) {
                    $record->load([
                        ...
                        'items.surveiHargas.revisiHargas.revisiKadivOpsApprover',
                    ...
                    $getScenarioDetails = function ($items) use ($record) {
                        ...
                        foreach ($groupedSurveys as $namaVendor => $surveys) {
                            ...
                            $allItemsCovered = true;
                            foreach ($items as $item) {
                                $survey = $surveys->where('item_id', $item->id)->first();
                                if (!$survey) {
                                    $allItemsCovered = false;
                                    break;
                                }
                        ...
                        foreach ($items as $item) {
                            $survey = $cheapestSurveys->where('item_id', $item->id)->first();
                            if (!$survey) continue;
                            $itemCost = $survey->harga * $item->kuantitas;
                    ...
                    if ($latestRevisi) {
                        ...
                        $formData['revisi_kadiv_ops_decision_type'] = $latestRevisi->revisi_kadiv_ops_decision_type;
                        $formData['revisi_kadiv_ops_catatan'] = $latestRevisi->revisi_kadiv_ops_catatan;
                        $formData['revisi_kadiv_ops_approver_name'] = $latestRevisi->revisiKadivOpsApprover?->nama_user;
            ...
            Action::make('edit_survey')
                ...
                ->mountUsing(function (Forms\Form $form, Pengajuan $record): void {
                    ...
                    $vendorPembayaran = VendorPembayaran::where('pengajuan_id', $record->id)->get()->keyBy('nama_vendor');
                    ...
                    foreach ($groupedByVendor as $namaVendor => $surveys) {
                        ...
                        foreach ($record->items as $item) {
                            $surveyForItem = $surveys->where('item_id', $item->id)->first();
            ...
            Action::make('penyelesaian')
                ...
                ->action(function (array $data, Pengajuan $record): void {
                    ...
                    if ($finalVendor instanceof \Illuminate\Database\Eloquent\Model) {
                        $finalVendor->update([
